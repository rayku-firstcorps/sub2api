...
var usageLogInsertArgTypes = [...]string{
	...
	"jsonb",       // request_context_json
	"boolean",     // request_context_truncated
	"integer",     // request_context_bytes
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			nullJSONRawMessage(log.RequestContextJSON),
			log.RequestContextTruncated,
			nullInt(log.RequestContextBytes),
			createdAt,
		},
	}
}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		requestContextJSON    sql.NullString
		requestContextTrunc   bool
		requestContextBytes   sql.NullInt64
		createdAt             time.Time
	)
	...
	log := &service.UsageLog{
		...
		RequestContextTruncated: requestContextTrunc,
	...
	if requestContextJSON.Valid {
		log.RequestContextJSON = &requestContextJSON.String
	}
	if requestContextBytes.Valid {
		value := int(requestContextBytes.Int64)
		log.RequestContextBytes = &value
	}
...
func nullJSONRawMessage(v *string) any {
